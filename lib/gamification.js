...
import clientPromise from "@/lib/mongodb";  // Added for optional raw support
...
function pointsForNextLevel(level) {
    // Level 1: 0 points, Level 2: 25, Level 3: 50, Level 4: 75, ...
    if (level <= 1) return 0;
...
function calculateLevelFromPoints(totalPoints) {
    // Find the highest level such that totalPoints >= pointsForNextLevel(level)
    let level = 1;
    while (totalPoints >= pointsForNextLevel(level + 1)) {
        level++;
    }
    return level;
}
...
// Optional: useRaw param for raw objects (uncomment if needed; uses clientPromise)
export async function updateStreak(userDoc, useRaw = false) {
    if (!userDoc || !userDoc._id) {
        throw new Error('Invalid user document provided');
    }
    ...
    // REMOVED: Second save() — awardPoints already saves

    console.log(`Streak updated for ${userDoc.email}: Day ${userDoc.streak}, +${streakPoints} points (consecutive: ${isConsecutive})`);
    return userDoc;
}

// Optional Raw Mode (uncomment if you want to use raw objects in callbacks/routes)
// export async function updateStreak(userDoc, useRaw = false) {
//     if (useRaw) {
//         // Raw logic: Use clientPromise for updateOne
//         const db = await clientPromise;
//         // ... (copy streak logic, then updateOne with $set)
//         // Example: await db.db().collection("users").updateOne({ _id: userDoc._id }, { $set: { streak: newStreak, ... } });
//         // Then call a raw awardPoints if needed
//         return updatedDoc;
//     }
//     // Else: Original Mongoose logic above
// }
